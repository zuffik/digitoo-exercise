...
export const ArticleDetailView: React.FC<Props> = (props: Props) => {
    ...
    const [{data: article, loading: loadingArticle}, refetchArticle] = useGet<ArticleDetailDTO>(`/articles/${params.id}`);
    const [{data: relatedArticles, loading: loadingRelatedArticles, error: errorRelatedArticles}] = useGet<List<Article>>(`/articles`);
    const [{loading: loadingNewComment}, postNewComment] = usePost<Comment>(`/comments`, {}, {manual: true});
    const [{loading: loadingVoteComment}, postVoteComment] = useAxios<Comment>({method: 'post'}, {manual: true});

    const onCommentPost = (comment: CreateComment) => postNewComment({
        data: {
    ...
    const onCommentDownVote = (commentId: string) => postVoteComment({url: `/comments/${commentId}/vote/down`});
    React.useEffect(() => {
