...
export const ArticleTable: React.FC<Props> = (props: Props) => {
    ...
    const columns = React.useMemo((): Column<Article>[] => [
        ...
        },
        {
            ...
            accessor: 'articleId',
            disableSortBy: true,
            Cell: ({value}) => <TableBasicActionButtons onEditClick={() => onEditArticleClick(value)} onRemoveClick={() => onRemoveArticleClick(value)}/>,
        } as Column<Article>
                    ...
                    {headerGroups.map(group => (
                        <tr {...group.getHeaderGroupProps()}>
                            {group.headers.map((col) => {
                                const sortByCol: UseSortByColumnProps<Article> = col as unknown as UseSortByColumnProps<Article>;
                                return (
                                    <th {...col.getHeaderProps(sortByCol.getSortByToggleProps())} className="text-nowrap align-top">
                                        <div className="d-flex flex-row justify-content-between">
                                            {col.render('Header')}
                                            {sortByCol.canSort && <TableSortInfo isSorted={sortByCol.isSorted} desc={sortByCol.isSortedDesc}/>}
                                        </div>
                                    </th>
                                );
                            })}
